...
import { useGetCurrentlyWachingShows } from "~/service/hooks/useGetShow";

export default function Screen() {
  const query = useGetHomePage();
  const currentlyWatchingShows = useGetCurrentlyWachingShows();
  ...
  return (
    <ScrollView>
      ...
      {currentlyWatchingShows.filter((i) => !!i).length > 0 && (
        <>
          <Text className="text-3xl font-semibold px-2 my-3">
            Continue Watching
          </Text>
          <View className="px-2">
            {query.isPending && <Skeleton className={"w-screen h-72"} />}
            {!query.isPending && currentlyWatchingShows && (
              <FlatList
                data={currentlyWatchingShows.filter((i) => !!i)}
                horizontal={true}
                renderItem={({ item, index }) => (
                  <Pressable
                    onPress={() => router.push(`/show/${item?.refShowId}`)}
                  >
                    <View
